...
public class LogUtil {
    ...
    private static final String TAG_DEFAULT = "consumermach";
    // 私有化，只调用静态方法
    private LogUtil() {
    }
    ...
    public static final String log_info = "/sdcard/ffst/dustbinbrain/log/";
    ...
    public static void writeBusinessLogOrException(String content, Throwable t){
        try {
            ...
            if(file.exists() && file.isDirectory()){
                // 目录大于100M则清空
                if(FileUtil.getLength(file) > 100 * 1024 *1024 && file.listFiles() !=null){
                    FileUtil.delete(file);
                }else{
                }
            }
            String filePath = log_info + TimeUtils.millis2String(System.currentTimeMillis(),"yyyy-MM-dd")+"_log.txt";
            LogUtil.v("记录日志,"+filePath+" content:"+content);
            if(t !=null ){
    ...
    private static void writeToFile(String filePath, String content){
        ...
        if(file.exists() && file.isFile()){
            ...
            if(file.length() >= 1024 * 1024){
                file.delete();
                LogUtil.e("文件大小超限, 删除:"+filePath);
            }
        }
        FileUtil.writeFileFromString(file,content,true);
    }
}